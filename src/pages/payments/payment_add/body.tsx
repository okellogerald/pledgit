...
import { paymentAddFormValuesStore } from "./store";
...
const validate = () => {
  const formState = paymentAddFormValuesStore.getState();
...
const addPayment = async () => {
  ...
  const data = paymentAddFormValuesStore.getState().getPaymentInput();
  return await PaymentsManager.instance.add(data);
};

export default function PaymentAddPageLoaderBody() {
  const formState = useStore(paymentAddFormValuesStore);
  ...
  const submit = async function () {
    const state = await trackPromise(addPayment(), setData);
