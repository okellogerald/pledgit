...
export interface PaymentEditFormState {
  ...
  setStartValue: (startValue: Payment) => void;
  ...
  getPaymentInput: () => PaymentEditInput;
...
export const paymentEditFormValuesStore = createStore<PaymentEditFormState>()(
  (set, get) => ({
    ...
    getPaymentInput() {
      ...
      if (!startValue) {
        throw new Error("No start value provided");
      }
    ...
    validate: () => {
      ...
      const pledge = pledgesStateStore.getState().pledges.filter((e) =>
        e.id === get().pledgeId
      );
      if (!pledge) {
    ...
    getPledgeLabel: (e) => {
      return `${e.contact.firstName} ${e.contact.lastName} (${e.campaign.name} ${
        formatTZAmount(e.amount)
      })`;
    },

    getPledgeSelectItem() {
      const pledge = pledgesStateStore.getState().pledges.find((e) =>
        e.id === get().pledgeId
      );
